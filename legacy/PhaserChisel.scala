package dsp

import chisel3._
import chisel3.util._

/**
 * Hardware implementation of a 4-Stage Phaser effect in Chisel,
 * modeled after the MXR Phase 90 analog phaser pedal.
 * Uses 32-bit Q1.31 fixed-point signed arithmetic.
 *
 * Architecture:
 *   - 24-bit LFO phase accumulator
 *   - 64-entry coefficient ROM (phase → all-pass coefficient 'a') with linear interpolation
 *   - 4 cascaded first-order All-Pass Filters evaluated in a single clock cycle (100% throughput)
 *   - Feedback path for resonance control
 *   - Dry/Wet mix at output
 *
 * @param rateHz     LFO sweep rate in Hz (default = 0.8 Hz)
 * @param minFreqHz  Minimum sweep frequency in Hz (default = 200.0)
 * @param maxFreqHz  Maximum sweep frequency in Hz (default = 2200.0)
 * @param feedback   Resonance feedback coefficient (default = 0.40)
 * @param mix        Dry/Wet blend (default = 0.50)
 * @param sampleRate Audio sampling rate in Hz (default = 44100.0)
 */
class PhaserChisel(
  val rateHz:     Double = 0.8,
  val minFreqHz:  Double = 200.0,
  val maxFreqHz:  Double = 2200.0,
  val feedback:   Double = 0.40,
  val mix:        Double = 0.50,
  val sampleRate: Double = 44100.0
) extends Module {
  val io = IO(new AudioStreamIO(32))

  // 1. Coefficient ROM: Pre-computed phase → all-pass coeff 'a'
  private val romEntries = 64
  private val coeffTable: Seq[BigInt] = (0 until romEntries).map { i =>
    val phase   = 2.0 * Math.PI * i.toDouble / romEntries.toDouble
    val lfoNorm = (Math.sin(phase) + 1.0) / 2.0
    val freq    = minFreqHz + lfoNorm * (maxFreqHz - minFreqHz)
    val t       = Math.tan(Math.PI * freq / sampleRate)
    val a       = (t - 1.0) / (t + 1.0)
    FixedPointQ31.doubleToQ31BigInt(a)
  }
  private val coeffROM = VecInit(coeffTable.map(_.S(32.W)))

  // 2. LFO Phase Accumulator (24-bit)
  private val phaseStepVal = BigInt(Math.round(rateHz * (1L << 24).toDouble / sampleRate))
  private val phaseStep    = phaseStepVal.U(24.W)

  val lfoPhaseReg = RegInit(0.U(24.W))

  // 3. Coefficient ROM Lookup with Linear Interpolation
  val romIdx     = lfoPhaseReg(23, 18)                         // 6-bit index [0..63]
  val romNextIdx = (romIdx +& 1.U)(5, 0)                       // wrap at 64
  val romFrac    = Cat(0.U(1.W), lfoPhaseReg(17, 3)).asSInt    // Q1.31-ish fraction

  val aY0        = coeffROM(romIdx)
  val aY1        = coeffROM(romNextIdx)
  val aDelta     = FixedPointQ31.subQ31(aY1, aY0)
  val aCoeffWire = FixedPointQ31.addQ31(aY0, FixedPointQ31.multQ31(aDelta, romFrac))

  // 4. Fixed-Point Constants (Q1.31)
  private val feedbackQ31    = FixedPointQ31.doubleToQ31BigInt(feedback).S(32.W)
  private val mixQ31         = FixedPointQ31.doubleToQ31BigInt(mix).S(32.W)
  private val oneMinusMixQ31 = FixedPointQ31.doubleToQ31BigInt(1.0 - mix).S(32.W)

  // 5. APF State Registers (4 stages × xPrev, yPrev)
  val xPrev = RegInit(VecInit(Seq.fill(4)(0.S(32.W))))
  val yPrev = RegInit(VecInit(Seq.fill(4)(0.S(32.W))))

  // 6. Registers
  val feedbackReg  = RegInit(0.S(32.W))
  val outSampleReg = RegInit(0.S(32.W))
  val outValidReg  = RegInit(false.B)

  // 7. All-Pass Filter Computation
  def apfCompute(x: SInt, a: SInt, xP: SInt, yP: SInt): SInt = {
    val ax  = FixedPointQ31.multQ31(a, x)     // a * x[n]
    val ayP = FixedPointQ31.multQ31(a, yP)    // a * y[n-1]
    val sum = FixedPointQ31.addQ31(ax, xP)    // a * x[n] + x[n-1]
    FixedPointQ31.subQ31(sum, ayP)            // a * x[n] + x[n-1] - a * y[n-1]
  }

  // 8. Single-Cycle 4-Stage Cascaded APF Pipeline
  when(io.sampleValid) {
    // Inject feedback
    val fbScaled    = FixedPointQ31.multQ31(feedbackReg, feedbackQ31)
    val inputWithFb = FixedPointQ31.addQ31(io.sampleIn, fbScaled)

    // Cascade 4 All-Pass Filter stages combinationally
    val st0 = apfCompute(inputWithFb, aCoeffWire, xPrev(0), yPrev(0))
    val st1 = apfCompute(st0,         aCoeffWire, xPrev(1), yPrev(1))
    val st2 = apfCompute(st1,         aCoeffWire, xPrev(2), yPrev(2))
    val st3 = apfCompute(st2,         aCoeffWire, xPrev(3), yPrev(3))

    // Update APF state registers for next sample
    xPrev(0) := inputWithFb
    yPrev(0) := st0

    xPrev(1) := st0
    yPrev(1) := st1

    xPrev(2) := st1
    yPrev(2) := st2

    xPrev(3) := st2
    yPrev(3) := st3

    feedbackReg := st3

    // Dry/Wet Mix
    val dryTerm  = FixedPointQ31.multQ31(oneMinusMixQ31, io.sampleIn)
    val wetTerm  = FixedPointQ31.multQ31(mixQ31, st3)
    val finalOut = FixedPointQ31.addQ31(dryTerm, wetTerm)

    lfoPhaseReg  := lfoPhaseReg + phaseStep
    outSampleReg := finalOut
    outValidReg  := true.B
  }.otherwise {
    outValidReg := false.B
  }

  // 9. Output
  io.sampleOut := outSampleReg
  io.outValid  := outValidReg
}