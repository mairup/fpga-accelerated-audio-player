...
class OverdriveChisel(
  val drive: Double = 2.0,
  val mix: Double = 0.8,
  val preGain: Double = 4.0
) extends Module {
  ...
  private val lutTable: Seq[BigInt] = (0 until lutEntries).map { i =>
    ...
    val driveX = drive * x
    val saturated = Math.tanh(driveX) + 0.08 * x * x
  ...
  // 4x Pre-gain with saturation
  val gained = Wire(SInt(32.W))
  val shifted = (io.sampleIn << 2).asSInt
  gained := FixedPointQ31.saturateSInt(shifted, 32)
