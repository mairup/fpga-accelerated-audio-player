...
class I2sController extends Module {
  ...
  val bclkFalling = !bclkSync && bclkPrev
  ...
  val txBitCounter = RegInit(0.U(7.W))
  val sdInReg = RegInit(false.B)
  ...
  when(bclkFalling) {
    val nextTxCounter = txBitCounter + 1.U
    txBitCounter := nextTxCounter
    when(nextTxCounter >= 2.U && nextTxCounter <= 33.U) {
      sdInReg := txShiftReg(31)
      txShiftReg := Cat(txShiftReg(30, 0), 0.U(1.W))
    }.otherwise {
      sdInReg := false.B
    }
  }
